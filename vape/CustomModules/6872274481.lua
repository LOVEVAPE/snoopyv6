...
local okConnectionBlock, _ = pcall(function()
	local chatFolder = ReplicatedStorage:FindFirstChild("DefaultChatSystemChatEvents")
	if not chatFolder then return end
	local onNew = chatFolder:FindFirstChild("OnNewMessage")
	if not onNew or onNew.ClassName ~= "RemoteEvent" then return end
	for i, v in pairs(getconnections(onNew.OnClientEvent)) do
		if
			v.Function
...
-- ignore if chat connections are not accessible
